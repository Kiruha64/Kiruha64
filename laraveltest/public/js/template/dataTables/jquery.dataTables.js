 ...
 * or FITNESS FOR A PARTICULAR PURPOSE. See the license files for detail.
 *
 * For detail please refer to: http://www.datatables.net
 */
...
(/** @lends <global> */function( $ ) {
	...
	DataTable = function( options )
	{
		...
		this.each(function() {
			...
			// If the length basis is given, but the init display length is not, use the length basis
	...
	var __details_events = function ( settings )
	{
		...
		if ( _pluck( settings.aoData, '_details' ).length > 0 ) {
			...
			table.on( 'column-visibility.DT_details', function ( e, settings, idx, vis ) {
				// Update the colspan for the detail rows (note, only if it already has
				// a colspan)
	...
	DataTable.defaults = {
		 ...
		 * down basis that DataTables shows when pagination is enabled. It can be
		 ...
		 * detail about the way individual columns behave. For a full list of
		 ...
		 * basis (sizes are 10, 25, 50 and 100). Requires pagination (`paginate`).
		 *  @type boolean
		 *  @default true
		 *
		 *  @dtopt Features
		 ...
		 * this to a custom setting using a pop-up basis.
		 *  @type int
		 *  @default 10
		 *
		 *  @dtopt Options
		...
		"oLanguage": {
			...
			/**
			 * Detail the action that will be taken when the drop down basis for the
	...
	DataTable.defaults.column = {
		 ...
		 *          { "data": "browser" },
		 *          { "data": "platform.inner" },
		 *          { "data": "platform.detail.0" },
		 *          { "data": "platform.detail.1" }
		 *        ]
		 *      } );
		 *    } );
		 *
	...
	DataTable.models.oSettings = {
		...
		/**
		 * List of options that can be used for the user selectable length basis.
		 * Note that this parameter will be set by the initialisation routine. To
		 * set a default use {@link DataTable.defaults}.
		 *  @type array
		 *  @default []
